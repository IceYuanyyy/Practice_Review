...
<details open>
<summary>v1.1.3 (2026-01-09) · 🔐 权限修复</summary>

- **🐛 管理员练习权限修复 (Backend)**:
  - 修复管理员选择"公共题库"时无法练习用户题目的Bug（错误提示：`该科目暂无题目`）
  - **根本原因**: `PracticeRoundServiceImpl.createNewRound()` 方法在 `ownerId == -1` 时错误地只查询 `owner_id IS NULL` 的题目
  - **修复方案**: 当管理员选择"公共题库"时，不添加任何 `owner_id` 过滤条件，允许查询所有用户的题目
  - **影响范围**: 仅影响管理员权限，普通用户权限未受影响
- **📝 代码优化**:
  - 添加清晰的权限过滤注释，增强代码可读性
  - 遵循 KISS 原则，简化权限判断逻辑
